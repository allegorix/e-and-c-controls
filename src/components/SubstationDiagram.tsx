...
export const SubstationDiagram: React.FC = () => {
  ...
  return (
    <section className="py-16 bg-slate-50/80 border-y border-slate-900 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          {stages.map((st, idx) => (
            <button
              ...
              className={`p-4 rounded-2xl border text-left transition duration-300 relative ${
                activeStage === idx
                  ? 'bg-white shadow-md border border-gray-100 border-orange-400 shadow-lg shadow-sm'
                  : 'bg-white shadow-md border border-gray-100 border-slate-200 hover:border-slate-200'
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-white text-orange-300 border border-blue-200">
                  {st.voltage}
                </span>
              </div>
        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-white shadow-md border border-gray-100 border border-blue-200 grid grid-cols-1 md:grid-cols-12 gap-6 items-center shadow-2xl">
          <div className="md:col-span-5 space-y-3">
            ...
            <h4 className="text-2xl font-black text-slate-900">{stages[activeStage].name}</h4>
            <p className="text-sm text-slate-600 leading-relaxed">{stages[activeStage].desc}</p>
          </div>

          <div className="md:col-span-7 bg-white shadow-md border border-gray-100 p-5 rounded-2xl border border-slate-200 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {stages[activeStage].components.map((comp, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-2 p-2.5 rounded-xl bg-white shadow-md border border-gray-100 border border-slate-200 text-xs text-slate-800"
                >
